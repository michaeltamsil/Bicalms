...
struct IndoorExerciseTrial : View {
    ...
    @State private var videoUrl: String = "D1 - Toy Soldiers"
    
    var body: some View {

        ZStack{
            
            LinearGradient(gradient: .init(colors: [(Color("Blue1")), (Color("Blue2"))]), startPoint: .top, endPoint: .bottom).edgesIgnoringSafeArea(.all)
            ...
            ZStack{
                ...
                IndoorAnimationTrial(videoUrl: videoUrl).frame(width: 360, height: 360, alignment: .center)
                    .clipShape(Circle())
            }
                ...
                VStack(spacing: 460){
                    ...
                    VStack(spacing: 4){
                     ...
                     .onReceive(timer) { (output) in
                        ...
                        if self.countDown < 5 && self.isPlayingExercise {
                            self.countDown -= 1
                        }
                        
                        if self.countDown == 0 && self.isPlayingExercise {
                            self.timer.upstream.connect().cancel()
                            self.videoUrl = "D7 - Lunge"
                        }
                    }
                }
                Spacer()
...
struct IndoorExerciseTrial_Previews: PreviewProvider {
